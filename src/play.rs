...
fn setup_song_queue(ids: Vec<&str>) -> io::Result<Vec<(String, String)>> {
    let mut b = Vec::new();

    for (index, id) in ids.iter().enumerate() {
        println!("{index} {id}");
        let dir = format!("music/{id}/out.ogg");
        b.insert(index, (id.to_string(), dir));
    }
...
pub fn play_music_new() {
    ...
    let (_, current_audio_file) = &audio_files[0];
    let mut sink = setup_sink(&stream_handle, current_audio_file, None);
    ...
    sink.play();
    println!("index: {current_audio_file_index}");
    loop {
        ...
        if now.elapsed() > converted_song_duration {
            ...
            sink = setup_sink(&stream_handle, current_audio_file, None);
            skip_amount = 0;
            sink.play();
            now = Instant::now();
        ...
        if keys.contains(&Keycode::Enter) && last_enter_press_time.elapsed() >= KEY_DEBOUNCE {
            ...
            sink = setup_sink(&stream_handle, current_audio_file, Some(skip));
            sink.play();
